...
# ================================
# Example Dapped-end beam, Case A
# Karin Yu, 2026

# Example C from Journal Paper "Grammar-based generation of strut-and-tie models for designing reinforced concrete structures" by K. Yu, M. A. Kraus, E. Chatzi, W. Kaufmann and  2026
...
example_name = 'DEB_SolA' # name of the example
...
reinf_list = np.array([[L_load, h_o+cnom, X_cross, h_o+cnom, 2*16**2*np.pi/4], # corner horizontal tie
                        [e_o+cnom, cnom, e_o+cnom, h-cnom, 2*18**2*np.pi/4], # corner vertical tie
                       [e_o+cnom, cnom, X_stirrup, cnom, 2*(20**2*np.pi/4)], # 1. horizontal tie
                       [X_stirrup, cnom, L+e_o-L_load, cnom, 2*(30**2*np.pi/4)], # 2. horizontal tie
