...
# ================================
# Example Dapped-end beam, Case C
# Karin Yu, 2026

# Example Case A from Journal Paper "Guided generation of strut-and-tie models for reinforced concrete structures with parametric graph grammatical evolution" by K. Yu, E. Chatzi, W. Kaufmann and M. A. Kraus 2026
...
example_name = 'DEB_SolC' # name of the example
...
polygon_out = []
add_x = [L_load, e_o*2+cnom, L-L_load+cnom]
add_y = []
...
reinf_list = np.array([[L_load, h-cnom, e_o*2+cnom, cnom, 20**2*np.pi/4*2], # diagonal tie
                       [e_o*2+cnom, cnom, L-L_load+cnom, cnom, 2*(26**2*np.pi/4)], # 1. horizontal tie
                       [L-L_load+cnom, cnom, L-L_load+e_o, cnom, 2*(30**2*np.pi/4)], # 2. horizontal tie
                       [L-L_load+cnom, cnom, L-L_load+cnom, h-cnom, 2*(16**2*np.pi/4)]]) # vertical tie
